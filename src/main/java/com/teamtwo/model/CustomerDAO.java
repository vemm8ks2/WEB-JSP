  ...
  public void update(CustomerDTO dto) {
    try {
      ...
      String sql = "UPDATE T_customer "
          + "SET customer_name = ?, customer_email = ?, customer_phone_number = ?, customer_birth = ?, customer_updated_at = ? "
          + "WHERE customer_id = ?";

      pstmt = conn.prepareStatement(sql);
      
      SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
      java.util.Date now = sdf.parse(dto.getCustomerUpdatedAt());
      Date sqlDate = new Date(now.getTime());

      pstmt.setString(1, dto.getCustomerName());
      pstmt.setString(2, dto.getCustomerEmail());
      pstmt.setString(3, dto.getCustomerPhoneNumber());
      pstmt.setString(4, dto.getCustomerBirth());
